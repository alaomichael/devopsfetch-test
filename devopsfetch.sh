...
# Function to calculate max column widths
calculate_max_widths() {
    ...
    while IFS='|' read -r -a columns; do
        for i in "${!columns[@]}"; do
            local length=${#columns[$i]}
            if (( length > widths[i] )); then
                widths[i]=$length
            fi
        done
    done <<< "$data"
    ...
    echo "$header" | sed -e "s/[^|]/-/g" | sed -e "s/|/-/g"
...
display_ports() {
    local port="$1"
    local header="Netid | State | Recv-Q | Send-Q | Local Address:Port | Peer Address:Port | Process | Service"
    local data
    
    if [ -n "$port" ]; then
        log_message "Displaying details for port $port"
        data=$(sudo ss -tunlp | grep ":$port " | awk '
            { printf "%s | %s | %s | %s | %s | %s | %s | %s\n", $1, $2, $3, $4, $5, $6, $7, $8 }')
    else
        log_message "Listing all active ports and services:"
        data=$(sudo ss -tunlp | awk '
            NR > 1 {
                printf "%s | %s | %s | %s | %s | %s | %s | %s\n", $1, $2, $3, $4, $5, $6, $7, $8
            }')
    fi
    ...
    calculate_max_widths "$header" "$data"
    echo "**************************************************************************************"
}

# Function to display Docker images and containers
display_docker() {
    local header_images="REPOSITORY | TAG | IMAGE ID | SIZE"
    local data_images
    local header_containers="NAMES | IMAGE | STATUS | PORTS"
    local data_containers

    log_message "Displaying Docker images:"
    data_images=$(docker images --format "{{.Repository}} | {{.Tag}} | {{.ID}} | {{.Size}}" | tail -n +2)
    echo "****************************** DOCKER STATUS ******************************"
    echo "Docker Images:"
    calculate_max_widths "$header_images" "$data_images"

    log_message "Displaying Docker containers:"
    data_containers=$(docker ps -a --format "{{.Names}} | {{.Image}} | {{.Status}} | {{.Ports}}" | tail -n +2)
    echo ""
    echo "Docker Containers:"
    calculate_max_widths "$header_containers" "$data_containers"
    echo "***************************************************************************"
}
...
display_nginx() {
    local domain="$1"
    local header_all="DOMAIN | PROXY | CONFIGURATION FILE"
    local data_all

    if [ -n "$domain" ]; then
        log_message "Displaying Nginx configuration for domain $domain"
        local domain_config
        domain_config=$(sudo nginx -T 2>/dev/null | awk "/server_name $domain/,/}/")
        ...
        data_all=$(sudo nginx -T 2>/dev/null | awk '
            /server_name/ {
                server_name = $2
                getline
                while ($0 !~ /}/) {
        ...
        calculate_max_widths "$header_all" "$data_all"
        echo "****************************************************************************"
    fi
}
...
display_users() {
    local username="$1"
    local header_users="Username | Full Name | Home Directory | Shell"
    local data_users
    local header_logins="Username | Terminal | Login Time | Session Duration"
    local data_logins

    if [ -n "$username" ]; then
        log_message "Displaying user details and login records for $username"
        data_users=$(getent passwd "$username" | awk -F: '{ printf "%s | %s | %s | %s\n", $1, $5, $6, $7 }')
        if [ -n "$data_users" ]; then
            echo "****************************** USER DETAILS ******************************"
            calculate_max_widths "$header_users" "$data_users"
            
            echo ""
            data_logins=$(last -w "$username" | head -n -2 | awk '{ printf "%s | %s | %s | %s\n", $1, $2, $4" "$5" "$6, $7 }')
            echo "Login Records:"
            calculate_max_widths "$header_logins" "$data_logins"
        else
            echo "No such user: $username"
        fi
    else
        log_message "Listing all users and their last login times"
        data_users=$(awk -F: '{ printf "%s | %s | %s | %s\n", $1, $5, $6, $7 }' /etc/passwd)
        echo "****************************** USER DETAILS ******************************"
        calculate_max_widths "$header_users" "$data_users"
        
        echo ""
        data_logins=$(for user in $(awk -F: '{print $1}' /etc/passwd); do last -w "$user" | head -n -2 | awk -v user="$user" 'NR > 1 { printf "%s | %s | %s | %s\n", user, $2, $4" "$5" "$6, $7 }'; done)
        echo "Login Records:"
        calculate_max_widths "$header_logins" "$data_logins"
    fi
    echo "**************************************************************************"
...
display_time_range() {
    local start_time="$1"
    local end_time="$2"
    local header="Username | Terminal | Login Time | Session Duration"
    local data
    ...
    data=$(last -w | awk -v start="$start_time" -v end="$end_time" '
